...
if __name__ == '__main__':
    ...
    teacher_env, get_logp_a, get_teacher_a, sess_teacher = load_policy_transfer('data1/', 29)

    teacher_size = teacher_env.obs_flat_size
    teacher_keys = teacher_env.obs_space_dict.keys()

    sess_teacher = tf.Session()
    sess_teacher.run(tf.global_variables_initializer())
    sess_teacher.run(target_init)

    # Sync params across processes
    sess_teacher.run(sync_all_params())

    # Setup model saving
    logger.setup_tf_saver(sess_teacher, inputs={'x': x_ph, 'a': a_ph},
                                outputs={'mu': mu, 'pi': pi, 'logp_a': logp_a, 'qr1': qr1, 'qr2': qr2, 'qc': qc})
    ...
    for i in range(100):
        ...
        positions = [teacher_env.robot_pos]
        while not(d or (ep_len == max_ep_len)):
            # Take deterministic actions at test time
            # a = get_teacher_a(o)
            a = get_action(o)
            o, r, d, info = teacher_env.step(a)
            ep_ret += r
            ep_cost += info.get('cost', 0)
            ep_len += 1
            ep_goals += 1 if info.get('goal_met', False) else 0
            positions.append(teacher_env.robot_pos)
